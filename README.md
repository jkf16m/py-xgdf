...
xg -w                  # xg-default (same as bare xg)
...
Workflows are selected with `-w`; there are no `workflow`, `cmd`, `run`, or
`pty` CLI commands. The built-ins are `xg-default` (alias `default`) and
